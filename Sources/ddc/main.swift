...
@main
struct DDCCommand: ParsableCommand {
  ...
  struct Detect: ParsableCommand {
    ...
    mutating func run() {
      let displays = DisplayFinder.findAll()
  ...
  struct Capabilities: ParsableCommand {
    ...
    @OptionGroup var options: DDCCommand.VCPOptions
  ...
  static func matchService(options: VCPOptions) -> [DDCDisplay] {
    let allDisplays = DisplayFinder.findAll()
    var matchedDisplays: [DDCDisplay] = []
  ...
  struct GetVCP: ParsableCommand {
    ...
    mutating func read(display: DDCDisplay) throws {
      let value = display.service?.read(command: UInt8(exactly: vcp)!)
      if !(value == nil) {
        ...
        if options.terse {
          ...
          // 特殊处理输入源 (VCP code 0x60 = 96)
          if vcp == 96, let input = commonInputs.first(where: { $0.value == UInt16(sl) }) {
            print(String(format: "VCP code 0x%02X (Input Source): %d (%@)", vcp, sl, input.name))
          } else {
            print(
              String(
                format:
  ...
  struct SetVCP: ParsableCommand {
    ...
    mutating func run() throws {
      ...
      let display = matchedDisplays[0]
      let writeOK = display.service?.write(
        command: UInt8(exactly: vcp)!, value: UInt16(exactly: value)!)
      if writeOK != true {
        throw DDCError.runtimeError("Failure when writing")
      } else if noVerify {
        print("Write OK")
      } else {
        let retrievedValues = display.service?.read(
          command: UInt8(exactly: vcp)!)
